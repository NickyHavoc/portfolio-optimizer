...
class MockStockGetter(StockGetter):
    def __init__(
        self,
    ...
    def sample_initial_price(self) -> float:
        return self.initial_price_range.sample()

    def sample_annual_return(self) -> float:
        return self.annual_return_range.sample()

    def sample_annual_stdev(self) -> float:
    ...
    def get_stock(
        ...
        _annual_return, _annual_stdev, _initial_price = (
            annual_return if annual_return is not None else self.sample_annual_return(),
            annual_stdev if annual_stdev is not None else self.sample_annual_stdev(),
            initial_price if initial_price is not None else self.sample_initial_price()
        )
